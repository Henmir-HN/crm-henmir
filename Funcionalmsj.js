...
async function callCrmTool(functionName, args) {
    const endpointMap = { /* ... tu mapa de endpoints ... */ };
...
// REEMPLAZA TU app.post('/api/whatsauto_reply', ...) ACTUAL CON ESTE BLOQUE 100% COMPLETO

app.post('/api/whatsauto_reply', async (req, res) => {
    // Lectura robusta de datos de la solicitud
    ...
    // 1. Verificar si el bot debe responder a este chat
    db.get('SELECT bot_active FROM conversations WHERE chat_id = ?', [chatId], async (err, row) => {
        if (err || (row && row.bot_active === 0)) {
            return res.json({ reply: "" }); // Respuesta vacía intencional
        }

        // 2. Si el bot está activo, procesar la conversación
        try {
            // Obtener configuración del bot (texto plano) desde SQLite
            ...
            const tools = [
                {
                    type: "function",
                    function: {
                        name: "search_vacancies_tool",
                        description: "Busca vacantes de empleo disponibles en el CRM, filtrando por ciudad y/o palabra clave.",
                        parameters: {
                            type: "object",
                            properties: {
                                city: { type: "string", description: "La ciudad para la búsqueda. Ej: 'San Pedro Sula'." },
                                keyword: { type: "string", description: "Palabra clave para el puesto. Ej: 'ventas', 'motorista'." }
                            },
                        },
                    },
                },
                {
                    type: "function",
                    function: {
                        name: "validate_registration_tool",
                        description: "Verifica en el CRM si un candidato se ha registrado recientemente usando su número de identidad. Se usa cuando el usuario confirma haber llenado el formulario.",
                        parameters: {
                            type: "object",
                            properties: {
                                identity: { type: "string", description: "El número de identidad (cédula) del candidato, sin guiones." },
                            },
                            required: ["identity"],
                        },
                    },
            ...
            // --- INICIO DEL CICLO DE FUNCTION CALLING ---
            ...
            if (toolCalls) {
                ...
                messagesForOpenAI.push(responseMessage); // Añadir la decisión de la IA al historial

                for (const toolCall of toolCalls) {
                    const functionName = toolCall.function.name;
                    const functionArgs = JSON.parse(toolCall.function.arguments);
                    
                    // Llamar a la herramienta real en nuestro CRM
                    ...
                    console.log(JSON.stringify(functionResponse, null, 2)); // Imprime el JSON bonito
                    ...
                    // --- ✨ LÓGICA DE TRANSICIÓN AUTOMÁTICA AQUÍ ✨ ---
                    // Si la herramienta que se llamó fue la de validación Y tuvo éxito...
                    if (functionName === 'validate_registration_tool' && functionResponse.success) {
                        console.log(`[${chatId}] Validación de registro exitosa. Desactivando bot.`);
                        
                        // Actualizamos la base de datos SQLite para desactivar el bot para este chat
                        db.run(`UPDATE conversations SET bot_active = 0 WHERE chat_id = ?`, [chatId], function(err) {
                            if (err) return console.error("Error al desactivar el bot en SQLite:", err.message);
                            // Creamos el registro si no existía (aunque es poco probable)
                            if (this.changes === 0) {
                                db.run(`INSERT INTO conversations (chat_id, bot_active) VALUES (?, 0)`, [chatId]);
                            }
                        });

                        // ✨ ENVIAR NOTIFICACIÓN EN TIEMPO REAL AL CRM ✨
                        if (crmSocket && crmSocket.readyState === WebSocket.OPEN) {
                            const notification = {
                                type: 'new_lead',
                                data: {
                                    chatId: chatId,
                                    candidateName: functionResponse.candidate_name,
                                    message: `Nuevo afiliado registrado: ${functionResponse.candidate_name}. La conversación requiere atención manual.`
                                }
                            };
            ...
            res.json({ reply: "Lo siento, estoy teniendo un problema técnico. Un asesor se pondrá en contacto pronto." });
        }
    });
});
